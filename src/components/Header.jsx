        ...
        <nav className='header_nav h-[70px] md:h-24 lg:h-32 xl:h-36 flex items-center justify-between'>
          ...
          <div className='flex items-center gap-4'>
            ...
            <div
              ...
              className={`${
                languageModal ? "border-white" : "border-transparent"
              } relative border-2 transition rounded-full flex items-center justify-center bg-white/90 size-[30px] sm:size-9 md:size-10 lg:size-11 xl:size-[50px] cursor-pointer`}
            >
              {
              ...
              {/* Language dropdown */}
              {
                languageModal && (
                  <div
                    ...
                    onMouseOver={() => setLanguageModal(true)}
                    onMouseLeave={() => setLanguageModal(false)}
                    className='absolute top-full mt-1 right-0 w-52 lg:w-64 rounded-xl overflow-hidden bg-white shadow-lg z-50'
                  >
                    <div onClick={() => handleTranslate("uz")} className='flex items-center gap-2 px-4 py-2 hover:bg-blue-600 group cursor-pointer transition'>
                      <img className='w-6 rounded-full' src={UZLogo} alt="Uzbekistan Flag" />
                      <p className='text-base lg:text-lg group-hover:text-white'>{t("language.uzbek")}</p>
                    </div>
                    <div onClick={() => handleTranslate("en")} className='flex items-center gap-2 px-4 py-2 hover:bg-blue-600 group cursor-pointer transition'>
                      <img className='w-6 rounded-full' src={ENLogo} alt="English Flag" />
                      <p className='text-base lg:text-lg group-hover:text-white'>{t("language.english")}</p>
                    </div>
                    <div onClick={() => handleTranslate("ru")} className='flex items-center gap-2 px-4 py-2 hover:bg-blue-600 group cursor-pointer transition'>
                      <img className='w-6 rounded-full' src={RULogo} alt="Russian Flag" />
                      <p className='text-base lg:text-lg group-hover:text-white'>{t("language.russian")}</p>
                    </div>
                  </div>
                )
              }
